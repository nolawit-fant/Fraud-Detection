...
class GeolocationAnalyzer:
    def __init__(self, fraud_data: pd.DataFrame, world_gdata: gpd.GeoDataFrame, logger: logging.Logger):
        """
        ...
        Args:
            fraud_data (pd.DataFrame): DataFrame containing fraud transaction data.
            world_gdata (gpd.GeoDataFrame): GeoDataFrame containing world geometries.
            logger (logging.Logger): Logger instance for logging information.
        """
        self.fraud_data = fraud_data
        self.world_gdata = world_gdata
        self.logger = logger
        self.fraud_rate_data = None
        self.transaction_volume_data = None
    ...
    def standardize_country_names(self):
        ...
        self.world_gdata['standardized_name'] = self.world_gdata['NAME'].apply(map_country_name)
        self.logger.info("Country names standardized successfully.")

    def calculate_fraud_rate(self):
        """
        Calculates the fraud rate by country and stores it in the fraud_rate_data attribute.
        ...
        total_by_country = self.fraud_data['country'].value_counts()
        fraud_by_country = self.fraud_data[self.fraud_data['class'] == 1]['country'].value_counts()
        ...
        self.fraud_rate_data = pd.DataFrame(fraud_rate_data)
        
        self.logger.info("Fraud rates calculated successfully.")
        return self.fraud_rate_data

    def calculate_transaction_volume(self):
        """
        Calculates the total transaction volume by country and stores it in the transaction_volume_data attribute.

        Returns:
            pd.DataFrame: A DataFrame with transaction volume by country.
        """
        self.logger.info("Calculating transaction volumes by country.")
        transaction_volume_by_country = self.fraud_data['country'].value_counts().reset_index()
        transaction_volume_by_country.columns = ['country', 'transaction_volume']
        self.transaction_volume_data = transaction_volume_by_country
        
        self.logger.info("Transaction volumes calculated successfully.")
        return self.transaction_volume_data
    ...
    def merge_data(self):
        ...
        self.world_fraud_map = self.world_gdata.merge(self.fraud_rate_data, how='left', left_on='standardized_name', right_on='country')
        self.world_fraud_map = self.world_fraud_map.merge(self.transaction_volume_data, how='left', on='country')
